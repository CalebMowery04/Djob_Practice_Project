<template>
    <div>
        <div class="py-20 px-6 bg-teal-700 text-center">
            <h1 class="mb-12 text-5xl text-white">Find a job anywhere</h1>

            <NuxtLink to="/browse" class="inline-block py-4 px-8 bg-teal-900 text-white rouded-xl">Browse</NuxtLink>
        </div>

        <div class="py-10 px-6">
            <h2 class="mb-8 text-2xl text-center">Newest Jobs</h2>
            
            <div class="space-y-4">
                <div class="p-6 flex items-center justify-between bg-gray-100 rounded-xl">
                    <div>
                        <h3 class="mb-2 text-xl font-semibold">The job position</h3>
                        <p class="text-gray-600">Company Name</p>
                    </div>

                    <div>
                        <p class="mb-2">Worldwide</p>
                        <p>$90-$120k</p>
                    </div>

                    <div>
                        <p>Posted Dec. 1. 2022</p>
                    </div>

                    <div>
                        <NuxtLink to="/browse/1" class="py-4 px-6 bg-teal-700 text-white rounded-x1">Details</NuxtLink>
                    </div>

                </div>

                <div class="p-6 flex items-center justify-between bg-gray-100 rounded-xl">
                    <div>
                        <h3 class="mb-2 text-xl font-semibold">The job position</h3>
                        <p class="text-gray-600">Company Name</p>
                    </div>

                    <div>
                        <p class="mb-2">Worldwide</p>
                        <p>$90-$120k</p>
                    </div>

                    <div>
                        <p>Posted Dec. 1. 2022</p>
                    </div>

                    <div>
                        <NuxtLink to="/browse/1" class="py-4 px-6 bg-teal-700 text-white rounded-x1">Details</NuxtLink>
                    </div>

                </div>

                <div class="p-6 flex items-center justify-between bg-gray-100 rounded-xl">
                    <div>
                        <h3 class="mb-2 text-xl font-semibold">The job position</h3>
                        <p class="text-gray-600">Company Name</p>
                    </div>

                    <div>
                        <p class="mb-2">Worldwide</p>
                        <p>$90-$120k</p>
                    </div>

                    <div>
                        <p>Posted Dec. 1. 2022</p>
                    </div>

                    <div>
                        <NuxtLink to="/browse/1" class="py-4 px-6 bg-teal-700 text-white rounded-x1">Details</NuxtLink>
                    </div>

                </div>
            </div>

        </div>
    </div>
</template>